...
$('textarea').on('keyup',function(){processWords();});
...
$(document).ready(function(){
    ...
    if( search.indexOf('?t=') !== -1 )
    {
        ...
        $('textarea').val(search.replace('\?t=','').replace(/\\n/g,"\n").replace(/%20/g,' '));
        processWords(words);
    }
    ...
    for( x in nomi )
    {
        dict_output += '<dt>'+x+'</dt><dd>'+nomi[x].replace(/\n/g,'<br>')+'</dd>'
    }
    $('#dict .well dl').html(dict_output);
});
...
function processWords(words)
{
    ...
    var output = '';
    var url = window.location.origin+'/toki-seme/?t=';
    ...
    output += '<br><br><p>URL to this translation: <a href="'+url+'">'+url+'</a></p>';
...
function translateWords(words,breaks)
{
    ...
    for( x in words )
    {
        if( words[x] != "" )
        {
            ...
            if( nomi[key] != undefined )
                output += '<button type="button" class="btn btn-default" data-toggle="tooltip" data-placement="bottom" title="'+nomi[key]+'">'+words[x]+' ('+nomi_single[key]+')</button>&nbsp;';
            else
                output += '<button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="Could not find match">'+words[x]+'</button>&nbsp;';
        }
